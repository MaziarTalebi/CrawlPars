...
class Esyoil2Pipeline(object):
    ...
    def Load_Dealer(self,company_name,siteid):
        ...
        try:
            ...
            result = -1000
            print(f"<problem>error in Load_Dealer is :: {e}")
        finally:
            db_curs.close()
            db_conn.close()
        return result
    ...
    def Update_Dealer(self,company_name,site_id):
        try:
            ...
            print(f"<problem>error in Update_Dealer is :: {e}")
        finally:
            db_curs.close()
            db_conn.close()
    ...
    def Insert_esyoil(self,item):
        if int(float(item['price'])) > 0:
            try:
                ...
                if fk_dealer == -1 :
                    print(f"couldnt find :: {item['providername']} we will update now")
                    self.Update_Dealer(item['providername'],item['esyoild_site_id'])
                    fk_dealer = self.Load_Dealer(item['providername'] , item['esyoild_site_id'])
                    print(f"we did update and get the id :: {fk_dealer}")
                    item['fk_dealer'] = fk_dealer
                elif fk_dealer >= 0:
                    print("only one found for :: {} ".format(item['providername']))
                    print(fk_dealer)
                    item['fk_dealer'] = fk_dealer
                elif fk_dealer == -1000:
                ...
                print("Greate these data has been successfully inserted:: {} , {} , {} , {} , {} , {} , {} , {} , {} , {} , {} , {} , {} , {} , {} , {} , {} , {} , {} , {} , {}".format(datetime.datetime.now(),item['cra_hash'],item['datetime'],item['esyoild_site_id'],item['fk_ans'],item['zipcode'],item['cra_amount'],item['up'],item['cra_pro'],item['payment_type'],item['cra_express'],item['cra_hose'],item['cra_short_vehicle'],item['providername'],item['deliverydate'],item['cra_position'],item['price'],item['Rate'],item['Reviews'],item['fk_dealer'],item['crawl_type']))
                db_conn.commit()
            except (MySQLdb.Error, MySQLdb.Warning) as e:
                print ("<problem>Time of the exception ERROR {}".format(datetime.datetime.now(),e))
